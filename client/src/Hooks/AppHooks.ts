...
import { SettingsUpdateModel } from "../Types/settings";

const savedStateJson = sessionStorage.getItem(settingsStorageKey);
const savedState = (
  savedStateJson ? JSON.parse(savedStateJson) : null
) as Settings | null;
...
export const useServerData = () => {
  ...
  useEffect(() => {
    async function asyncCall() {
      const settingsState = (savedState ??
        (await fetchSettings())) as SettingsUpdateModel;
